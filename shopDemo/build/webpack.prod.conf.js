...
const env = process.env.NODE_ENV === 'testing'
  ...
  : require('../config/prod.env')//如果不是测试环境就直接使用生产环境
...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    //把webpack的runtime和manifest这些webpack管理所有模块交互的代码打包到[name].js文件中,
    // 防止build之后vendor的hash值被更新
    new webpack.optimize.CommonsChunkPlugin({
      name: 'app',
      async: 'vendor-async',
      children: true,
...
if (config.build.productionGzip) {
  ...
  webpackConfig.plugins.push(
    ...
    // https://doc.webpack-china.org/plugins/compression-webpack-plugin/
